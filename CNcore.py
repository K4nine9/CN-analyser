...
# isMediaNote	Int	User-entered checkbox in response to question “Is your note about the Tweet or the image?”. New as of 2023-05-24.	1 if “About the image in this Tweet, and should appear on all Tweets that include this image” is selected, and 0 otherwise (including both if "About this specific Tweet" is selected instead, or by default, e.g. if the note was written on a Tweet without media).
# isCollaborativeNote	Int	Binary indicator indicating if the note is a collaborative note. New as of 2026-02-04	1 if the note is a collaborative note. 0 if the note is a regular note (default).

class NoteData:
    def __init__(self, noteID:int, noteAuthorParticipantID:str, createdAtMillis:int, tweetID:int, classification:str, believable:str, harmful:str, validationDifficulty:str, misleadingOther:int, misleadingFactualError:int, misleadingManipulatedMedia:int, misleadingOutdatedInformation:int, misleadingMissingImportantContext:int, misleadingUnverifiedClaimAsFact:int, misleadingSatire:int, notMisleadingOther:int, notMisleadingFactuallyCorrect:int, notMisleadingOutdatedButNotWhenWritten:int, notMisleadingClearlySatire:int, notMisleadingPersonalOpinion:int, trustworthySources:int, summary:str, isMediaNote:int, isCollaborativeNote:int):
        self.noteID = noteID
...
class NoteStatusHistoryData:
    def __init__(self, noteId:int, participantId:str, createdAtMillis:int, timestampMillisOfFirstNonNMRStatus:int, firstNonNMRStatus:str, timestampMillisOfCurrentStatus:int, currentStatus:str, timestampMillisOfLatestNonNMRStatus:int, latestNonNMRStatus:str, timestampMillisOfStatusLock:int, lockedStatus:str, timestampMillisOfRetroLock:int, currentCoreStatus:str, currentExpansionStatus:str, currentGroupStatus:str, currentDecidedByKey:str, currentModelingGroup:int, timestampMillisOfMostRecentStatusChange:int, timestampMillisOfNmrDueToMinStableCrhTime:int, currentMultiGroupStatus:str, currentModelingMultiGroup:int, timestampMinuteOfFinalScoringOutput:int, timestampMillisOfFirstNmrDueToMinStableCrhTime:int):
        self.noteId = noteId
        self.participantId = participantId
        self.createdAtMillis = createdAtMillis
        self.timestampMillisOfFirstNonNMRStatus = timestampMillisOfFirstNonNMRStatus
        self.firstNonNMRStatus = firstNonNMRStatus
        self.timestampMillisOfCurrentStatus = timestampMillisOfCurrentStatus
        self.currentStatus = currentStatus
        self.timestampMillisOfLatestNonNMRStatus = timestampMillisOfLatestNonNMRStatus
        self.latestNonNMRStatus = latestNonNMRStatus
        self.timestampMillisOfStatusLock = timestampMillisOfStatusLock
        self.lockedStatus = lockedStatus
        self.timestampMillisOfRetroLock = timestampMillisOfRetroLock
        self.currentCoreStatus = currentCoreStatus
        self.currentExpansionStatus = currentExpansionStatus
        self.currentGroupStatus = currentGroupStatus
        self.currentDecidedByKey = currentDecidedByKey
        self.currentModelingGroup = currentModelingGroup
        self.timestampMillisOfMostRecentStatusChange = timestampMillisOfMostRecentStatusChange
        self.timestampMillisOfNmrDueToMinStableCrhTime = timestampMillisOfNmrDueToMinStableCrhTime
        self.currentMultiGroupStatus = currentMultiGroupStatus
        self.currentModelingMultiGroup = currentModelingMultiGroup
        self.timestampMinuteOfFinalScoringOutput = timestampMinuteOfFinalScoringOutput
        self.timestampMillisOfFirstNmrDueToMinStableCrhTime = timestampMillisOfFirstNmrDueToMinStableCrhTime
...
# helpful	Int	Binary indicator, based on user-entered multiple choice in response to note rating question “Is this note helpful? ” - Deprecated as of 2021-06-30.	1 if “Yes” is selected, 0 if “No” is selected
# notHelpful	Int	Binary indicator, based on user-entered multiple choice in response to note rating question “Is this note helpful?” Deprecated as of 2021-06-30.	1 if “No” is selected, 0 if “Yes” is selected
# helpfulnessLevel	String	User-entered multiple choice response to note rating question: “Is this note helpful” Added as of 2021-06-30.	"NOT_HELPFUL" "SOMEWHAT_HELPFUL" "HELPFUL"
...
# helpfulInformative	Int	User-entered checkbox in response to question “What about this note was helpful to you?” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Informative” is selected, else 0.
...
# helpfulEmpathetic	Int	User-entered checkbox in response to question “What about this note was helpful to you?” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Nonjudgmental and/or empathetic” is selected, else 0.
...
# helpfulUniqueContext	Int	User-entered checkbox in response to question “What about this note was helpful to you?” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Offers unique information or context” is selected, else 0.
# helpfulAddressesClaim	Int	User-entered checkbox in response to question “What was helpful about it?” (Check all that apply question type). New as of 2021-06-30	1 if “Directly addresses the Tweet's claim” is selected, else 0.
# helpfulImportantContext	Int	User-entered checkbox in response to question “What was helpful about it?” (Check all that apply question type). New as of 2021-06-30	1 if “Provides important context” is selected, else 0.
# helpfulUnbiasedLanguage	Int	User-entered checkbox in response to question “What was helpful about it?” (Check all that apply question type). New as of 2021-12-15	1 if “Neutral or unbiased language” is selected, else 0.
...
# NotHelpfulOpinionSpeculationOrBias	Int	User-entered checkbox in response to prompt “Help us understand why this note was unhelpful” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Opinion, speculation, or bias” is selected, else 0.
...
# notHelpfulOutdated	Int	User-entered checkbox in response to prompt “Help us understand why this note was unhelpful” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Outdated information” is selected, else 0.
...
# notHelpfulArgumentativeOrBiased	Int	User-entered checkbox in response to prompt “Help us understand why this note was unhelpful” (Check all that apply question type). Variable name changed from notHelpfulArgumentativeOrInflammatory in 2021-12-15	1 if “Argumentative or biased language is selected, else 0.
# notHelpfulOffTopic	Int	User-entered checkbox in response to prompt “Help us understand why this note was unhelpful” (Check all that apply question type). Deprecated as of 2021-06-30.	1 if “Off topic” is selected, else 0.
...
# ratedOnTweetId	Long	The unique ID of the Tweet that the note was rated on (which, in the case of media notes, may not be the same Tweet as the note was originally written on). New as of 2023-05-24.	
# ratingSourceBucketed	String	The source of the rating (ratings are population-sampled only if they are made in response to a "Needs Your Help" notification). New as of 2025-09-30.	"DEFAULT", "POPULATION_SAMPLED"
# suggestion	String	User-entered suggestion string (for collaborative notes). New as of 2026-02-04.

class RatingData:
    def __init__(self, noteId: int, participantId: str, createdAtMillis: int, ratedOnTweetId: int, ratingSourceBucketed: str, suggestion: str, helpfulOther: int, helpfulInformative: int, helpfulClear: int, helpfulEmpathetic: int, helpfulGoodSources: int, helpfulUniqueContext: int, helpfulAddressesClaim: int, helpfulImportantContext: int, helpfulUnbiasedLanguage: int, notHelpfulOther: int, notHelpfulIncorrect: int, notHelpfulSourcesMissingOrUnreliable: int, NotHelpfulOpinionSpeculationOrBias: int, notHelpfulMissingKeyPoints: int, notHelpfulOutdated: int, notHelpfulHardToUnderstand: int, notHelpfulArgumentativeOrBiased: int, notHelpfulOffTopic: int, notHelpfulSpamHarassmentOrAbuse: int, notHelpfulIrrelevantSources: int, notHelpfulOpinionSpeculation: int, notHelpfulNoteNotNeeded: int):
        self.noteId = noteId
        self.participantId = participantId
        self.createdAtMillis = createdAtMillis
        self.ratedOnTweetId = ratedOnTweetId
        self.ratingSourceBucketed = ratingSourceBucketed
        self.suggestion = suggestion
        self.helpfulOther = helpfulOther
        self.helpfulInformative = helpfulInformative
        self.helpfulClear = helpfulClear
        self.helpfulEmpathetic = helpfulEmpathetic
        self.helpfulGoodSources = helpfulGoodSources
        self.helpfulUniqueContext = helpfulUniqueContext
        self.helpfulAddressesClaim = helpfulAddressesClaim
        self.helpfulImportantContext = helpfulImportantContext
        self.helpfulUnbiasedLanguage = helpfulUnbiasedLanguage
        self.notHelpfulOther = notHelpfulOther
        self.notHelpfulIncorrect = notHelpfulIncorrect
        self.notHelpfulSourcesMissingOrUnreliable = notHelpfulSourcesMissingOrUnreliable
        self.NotHelpfulOpinionSpeculationOrBias = NotHelpfulOpinionSpeculationOrBias
        self.notHelpfulMissingKeyPoints = notHelpfulMissingKeyPoints
        self.notHelpfulOutdated = notHelpfulOutdated
        self.notHelpfulHardToUnderstand = notHelpfulHardToUnderstand
        self.notHelpfulArgumentativeOrBiased = notHelpfulArgumentativeOrBiased
        self.notHelpfulOffTopic = notHelpfulOffTopic
        self.notHelpfulSpamHarassmentOrAbuse = notHelpfulSpamHarassmentOrAbuse
        self.notHelpfulIrrelevantSources = notHelpfulIrrelevantSources
        self.notHelpfulOpinionSpeculation = notHelpfulOpinionSpeculation
        self.notHelpfulNoteNotNeeded = notHelpfulNoteNotNeeded
...
class UserEnrollmentData:
    def __init__(self, participantId: str, enrollmentState: str, successfulRatingNeededToEarnIn: int, timestampOfLastStateChange: int, timestampOfLastEarnOut: int, modelingPopulation: str, modelingGroup: int):
        self.participantId = participantId
        self.enrollmentState = enrollmentState
        self.successfulRatingNeededToEarnIn = successfulRatingNeededToEarnIn
        self.timestampOfLastStateChange = timestampOfLastStateChange
        self.timestampOfLastEarnOut = timestampOfLastEarnOut
        self.modelingPopulation = modelingPopulation
        self.modelingGroup = modelingGroup
