...
def process_tsv_in_chunks(file_path: str, data_class, insert_func, db_path: str, chunk_size: int = 10000, encoding="utf-8") -> None:
    print(f"File: {file_path} will be processed in chunks of {chunk_size}")
    conn = sq3.connect(db_path)
    cursor = conn.cursor()
    ...
    try:
        with open(file_path, "r", encoding=encoding) as f:
            ...
            batch = []
            for row in tqdm(reader, desc=f"Processing {os.path.basename(file_path)}"):
                try:
...
def insert_note_data(cursor, note_data_list: List[NoteData]) -> None:
    values = [(
...
def insert_note_status_history(cursor, data_list: List[NoteStatusHistoryData]) -> None:
    values = [(
...
def insert_ratings(cursor, data_list: List[RatingsData]) -> None:
    values = [(
...
def insert_user_enrollment(cursor, data_list: List[UserEnrollmentData]) -> None:
    values = [(
...
def insert_bat_signals(cursor, data_list: List[BatSignalsData]) -> None:
    values = [(
...
def main() -> None:
    ...
    #     process_tsv_in_chunks(path, RatingsData, insert_ratings, db_path)

    # print("Processing user enrollment data...")
    # for path in user_enrollment_path_list:
    #     process_tsv_in_chunks(path, UserEnrollmentData, insert_user_enrollment, db_path)

    # print("Processing bat signals data...")
    # for path in bat_signals_path_list:
    #     process_tsv_in_chunks(path, BatSignalsData, insert_bat_signals, db_path)
